...
Функция ОбъектКомпоненты() Экспорт
	...
	//
	Возврат слкМенеджерЗащитыПовтИсп.ОбъектКомпоненты();
КонецФункции
...
//	ТолькоНаличиеКлюча - Булево - Контролировать только наличие ключа
//                 
Процедура ИнициализироватьСериюЗащиты(Знач Серия, ТолькоНаличиеКлюча = Ложь) Экспорт
...
//
Функция ЗагрузитьФайлыДанных(Знач Серия, МенеджерСерииЗащиты, МенеджерОбъектов, Знач ОбновитьСуществующие = Истина) Экспорт
	
	ФайлыДанных = МенеджерСерииЗащиты.ПараметрыПодключения.ФайлыДанных;
	Для Каждого ФайлДанных Из ФайлыДанных Цикл
		Если Не МенеджерОбъектов.ПопыткаМакетЗагружен(ФайлДанных.Ключ) Тогда
			Если Не МенеджерОбъектов.ПопыткаЗагрузитьИзМакета(ПолучитьОбщийМакет(ФайлДанных.Ключ), ФайлДанных.Ключ, ОбновитьСуществующие) Тогда
...
//
Функция ПараметрыСвязи(Знач Host = "", Знач Port = "") Экспорт
