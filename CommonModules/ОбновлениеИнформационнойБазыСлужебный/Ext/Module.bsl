...
Функция ОбработчикиОбновленияВИнтервале(Знач ИсходнаяТаблицаОбработчиков, Знач ВерсияОт, Знач ВерсияДо, 
	...
	Схема = ПолучитьОбщийМакет("ПолучениеДереваОбработчиковОбновленияДМ");
...
Функция ИтерацииОбновления()
	...
	Для каждого ИмяПодсистемы Из ОписанияПодсистем.Порядок Цикл
		...
		Если ОписаниеПодсистемы.Имя = ИмяОсновнойКонфигурации Тогда
			ОбновлениеИнформационнойБазыДМ.ИзменитьРазрядностьНомеровВерсийДляОбработчиков( ИтерацияОбновления.Обработчики );
...
Функция ДокументОписаниеОбновлений(Знач Разделы) Экспорт
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		МакетОписаниеОбновлений = ОбновлениеИнформационнойБазыДМ.ПолучитьМакетОписаниеОбновлений(МакетОписаниеОбновлений);
	Иначе
...
Процедура ВывестиОписаниеИзменений(Знач НомерВерсии, ДокументОписаниеОбновлений, МакетОписаниеОбновлений)
	...
	Если Прав( НомерВерсии, 2 ) = ".0" Тогда
		Номер = Лев(НомерВерсии, СтрДлина(НомерВерсии)-2);
	Иначе
		Номер = НомерВерсии;
	КонецЕсли;
	Номер = СтрЗаменить(Номер, ".", "_");
...
Функция РазделыОписанияИзменений() Экспорт
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		...
		ПредикатШапки = "Шапка";
		Макет = ОбновлениеИнформационнойБазыДМ.ПолучитьМакетОписаниеОбновлений(МакетОписаниеОбновлений);
		...
		Для каждого Область Из Макет.Области Цикл
			...
			Пока РазрядыВерсииСтроками.Количество() < 5 Цикл
				РазрядыВерсииСтроками.Добавить("0");
			КонецЦикла;
			Если РазрядыВерсииСтроками.Количество() > 5 Тогда
				Продолжить;
			КонецЕсли;
...
Функция ВесВерсииИзМассиваСтрок(РазрядыВерсииСтроками)
	...
	Возврат 0
		+ Число(РазрядыВерсииСтроками[0]) * 1000000000000
		+ Число(РазрядыВерсииСтроками[1]) * 1000000000
		+ Число(РазрядыВерсииСтроками[2]) * 1000000
		+ Число(РазрядыВерсииСтроками[3]) * 1000
		+ Число(РазрядыВерсииСтроками[4]);
