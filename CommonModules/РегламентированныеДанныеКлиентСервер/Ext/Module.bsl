...
//  СтранаРегистрации  - СправочникСсылка - ссылка на справочник типа:
//											СправочникСсылка.СтраныМира.
//
// Возвращаемое значение:
//  Булево - Истина, если соответствует.
//
Функция ИННСоответствуетТребованиям(Знач ИНН, ЭтоЮридическоеЛицо, ТекстСообщения, СтранаРегистрации = Неопределено) Экспорт
	...
	// Для России известны правила формирования ИНН для стран ЕАЭС известно только кол-во цифр, по остальным неизвестно
	Если СтранаРегистрации = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
		...
		КонецЕсли;
		//<УВС	
	ИначеЕсли ВРЕГ(СтранаРегистрации) = "БЕЛАРУСЬ" И ДлинаИНН <> 9 Тогда
		
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'ИНН должен состоять из 9 цифр.'");
	...
	ИначеЕсли ВРЕГ(СтранаРегистрации) = "АРМЕНИЯ" И ДлинаИНН <> 8 Тогда
		
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'ИНН должен состоять из 8 цифр.'");
	...
	ИначеЕсли ВРЕГ(СтранаРегистрации) = "КАЗАХСТАН" И ДлинаИНН <> 12 Тогда
		
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'ИНН должен состоять из 12 цифр.'");
	...
	ИначеЕсли ВРЕГ(СтранаРегистрации) = "КИРГИЗИЯ" И ДлинаИНН <> 14 Тогда
		
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'ИНН должен состоять из 14 цифр.'");
