...
&НаКлиенте
Процедура ТаблицаТООбновлениеТранспортаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя <> "ТаблицаТООбновлениеТранспортаОбновленныйТранспорт" Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ТО",Объект.ТаблицаТООбновлениеТранспорта.Получить(ВыбраннаяСтрока).ТО);
		ПараметрыФормы.Вставить("АдресТЗТаблицаМаршрутСледования",ПолучитьАдресТЗ("ТаблицаМаршрутСледования"));
		ПараметрыФормы.Вставить("ВнесениеВторогоТранспортаБезАннулирования",Ложь);
		ОткрытьФорму("Обработка.ДМ_ВнесениеДанныхОТранспорте.Форма.ФормаМаршрутаСледования", ПараметрыФормы, ЭтотОбъект);
		СтандартнаяОбработка = Ложь;
		ТаблицаТООбновлениеТранспортаПриАктивизацииСтроки();
	КонецЕсли;
...
Функция ПроверкаЗаполненияТранспортом()
	...
	Если Объект.ТаблицаТООбновлениеТранспорта.Количество() > 0 Тогда
		Для Каждого ТекСтр1 Из Объект.ТаблицаТООбновлениеТранспорта Цикл
			Если Не ЗначениеЗаполнено(ТекСтр1.ОбновленныйТранспорт) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не везде указан обновленный транспорт");
				Возврат Ложь;
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;	
	...
	Для Каждого ТекСтр1 Из Объект.ТаблицаТООбновлениеТранспорта Цикл
		...
		НовСтр1.ИсходныйТранспорт = ТекСтр1.ТО.Транспорт;
		НовСтр1.ОбновленныйТранспорт = ТекСтр1.ОбновленныйТранспорт;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для м=0 по Параметры.МассивОтобранныхДокументов.Количество()-1 Цикл
		...
		ИначеЕсли Не ПроверитьСтатусыВСД(ИсхТО) Тогда
			Продолжить;
		КонецЕсли;
		НовСтр = Объект.ТаблицаТООбновлениеТранспорта.Добавить();
		НовСтр.ТО = ИсхТО;
		НовСтр.ИсходныйТранспорт = ИсхТО.Транспорт;
...
&НаКлиенте
Процедура ТаблицаТООбновлениеТранспортаПередУдалением(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры
...
&НаКлиенте
Процедура ТаблицаТООбновлениеТранспортаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
КонецПроцедуры
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.ДМ_ВнесениеДанныхОТранспорте.Форма.ФормаМаршрутаСледования") Тогда
		...
		Для Каждого ТекСтр из Объект.ТаблицаТООбновлениеТранспорта Цикл
...
Процедура СкопироватьМаршрутСледования(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("СкопироватьМаршрутСледованияПослеОтвет", ЭтотОбъект,Элементы.ТаблицаТООбновлениеТранспорта.ТекущиеДанные.ТО);
	ТекстВопроса = НСтр("ru = 'Скопировать маршрут следования из текущей строки с док-м " + Элементы.ТаблицаТООбновлениеТранспорта.ТекущиеДанные.ТО + " для всех остальных строк ?'");
	ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаТООбновлениеТранспортаПриАктивизацииСтроки()
	
	Элементы.СкопироватьМаршрутСледования.Доступность = Не ПустаяСтрока(Элементы.ТаблицаТООбновлениеТранспорта.ТекущиеДанные.ТаблицаМаршрутСледования);
