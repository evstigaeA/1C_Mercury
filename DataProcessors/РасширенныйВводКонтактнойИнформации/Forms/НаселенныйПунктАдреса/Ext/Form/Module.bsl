...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//<УВС
	Параметры.Свойство("Страна", Страна);
	//>УВС
...
Процедура ПостроитьФормуПоЧастямАдреса()
	...
	Для Каждого КлючЗначение Из ЧастиАдреса Цикл
		...
		//<УВС
		Если Часть.Уровень = 2 ИЛИ Часть.Уровень = 5 Тогда
			Продолжить;
		КонецЕсли;
		//>УВС
		...
		Если Часть.Уровень < 7 И НЕ (ФорматАдреса = "КЛАДР" И (Часть.Уровень = 2 ИЛИ Часть.Уровень = 5))Тогда
			...
			//<УВС
			Если Часть.Уровень = 1 Тогда
				Элемент.РедактированиеТекста = Ложь;
				Элемент.ПодсказкаВвода = "Используйте кнопку выбора";
			КонецЕсли;
			//>УВС
...
Процедура Подключаемый_ПолеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	УровеньАдреса = 0;//УВС УровеньАдреса необходим для хранения уровня родителя (важно при получении данных по городу)
	...
	ПараметрыОткрытия.Вставить("Родитель", ИдентификаторРодителяЧастиАдресаЭлемента(ЧастьАдреса, ЧастиАдреса, Страна, УровеньАдреса));//УВС УровеньАдреса
	ПараметрыОткрытия.Вставить("УровеньАдреса", УровеньАдреса);
...
&НаСервереБезКонтекста
Функция ИдентификаторРодителяЧастиАдресаЭлемента(ЧастьАдреса, ЧастиАдреса, Страна, УровеньАдреса) //УВС страна, УровеньАдреса
	...
	Иначе
		ЕстьЧастьАдресаБезИдентификатора = ЕстьЛиИдентификаторРодителяЧастиАдресаЭлемента(ЧастиАдреса, ЧастьАдреса, Идентификатор, УровеньАдреса);//УВС УровеньАдреса
...
&НаСервереБезКонтекста
Функция ЕстьЛиИдентификаторРодителяЧастиАдресаЭлемента(ЧастиАдреса, ЧастьАдреса, Идентификатор, УровеньАдреса)//УВС УровеньАдреса
