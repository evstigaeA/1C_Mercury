...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОсновнаяСтрана           = Параметры.Страна;
	//>УВС
	...
	Если Контекст.НаселенныйПунктДетально <> Неопределено Тогда
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			Если Не ПустаяСтрока(ЧастьПункта.Представление) Тогда
				...
				//<УВС
				ЗаполнитьИдентификаторыОбъектаXDTO(АдресРФ, КлючЗначение.Ключ, КлючЗначение.Значение.Идентификатор);
				//>УВС
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьКонтекстПоXDTOАдреса(XDTOДанныеАдреса, Контекст, Уровни = Неопределено)
	...
	Если АдресРФ <> Неопределено Тогда
		...
		//<УВС
		СвойстваАдресРФ = АдресРФ.Свойства();
		//>УВС
		...
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			Если Уровни = Неопределено ИЛИ Уровни.Найти(Значение.Уровень) <> Неопределено Тогда
				...
				//<УВС
				СвойствоXDTO = "ID" + КлючЗначение.Ключ;
				Если СвойстваАдресРФ.Получить(СвойствоXDTO) <> Неопределено Тогда
					Если АдресРФ[СвойствоXDTO] <> Неопределено Тогда
						Значение.Идентификатор = Новый УникальныйИдентификатор(АдресРФ[СвойствоXDTO]);
					КонецЕсли;
				КонецЕсли;
				//>УВС
			КонецЕсли;
...
Процедура ПроверитьДоступностьКлассификатора()
	...
	РезультатПроверки = ПроверитьДоступностьСервиса();
	
	Если РезультатПроверки = Неопределено Тогда
		Элементы.ГруппаОписаниеНедоступностиСервера.ТекущаяСтраница = Элементы.СервисНедоступен;
		ТекстСообщенияСервиса = НСтр("ru = 'Работа с ИС ""Меркурий"" не возможна.'");
	ИначеЕсли РезультатПроверки.Отказ Тогда
		Элементы.ГруппаОписаниеНедоступностиСервера.ТекущаяСтраница = Элементы.СервисНедоступен;
		ТекстСообщенияСервиса = НСтр("ru = 'Получение адресной информации из Меркурия недоступно:'") + Символы.ПС 
...
//<УВС
&НаСервереБезКонтекста
Процедура ЗаполнитьИдентификаторыОбъектаXDTO(ОбъектXDTO, Путь, Значение)
	
	Если Значение = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектXDTO["ID" + Путь] = Строка(Значение);
	
КонецПроцедуры//>УВС

//<УВС
&НаСервереБезКонтекста
Функция ПроверитьДоступностьСервиса()
	
	РезультатПроверки = Новый Структура;
	РезультатПроверки.Вставить("Отказ", Ложь);
	Возврат РезультатПроверки;
	//Возврат ОбменУВССервер.ТестовыйВызовСервиса();
