...
Процедура СписокКлючей(Команда)
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		слкМенеджерЗащиты.СообщитьОбОшибкеСЛК(ОписаниеОшибки, ТекущаяСерия.Серия);
	Иначе
		ПоказатьПредупреждение(,"Список установленных на сервере СЛК ключей серии " + ТекущаяСерия.Серия + ": " + Символы.ПС + СписокКлючей);
	КонецЕсли;
...
Процедура КоличествоЛицензий(Команда)
	...
	ЗначенияСчетчиковЛицензий = слкМенеджерЗащиты.ПолучитьЗначенияСчетчиковЛицензий(ТекущаяСерия.Серия);
	
	ПоказатьПредупреждение(,"Количество лицензий ключей серии " + ТекущаяСерия.Серия + ":
		|	- общее: " + ЗначенияСчетчиковЛицензий.TotalLicenceCount + "
		|	- использованных: " + ЗначенияСчетчиковЛицензий.UseLicenceCount + "
		|	- свободных: " + ЗначенияСчетчиковЛицензий.FreeLicenceCount
		);
