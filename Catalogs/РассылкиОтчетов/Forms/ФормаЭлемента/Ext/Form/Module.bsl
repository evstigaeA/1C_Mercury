...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	// ALEV
	//ТабДок = Новый ТабличныйДокумент;
	//ТабМакет = Справочники.РассылкиОтчетов.ПолучитьМакет("МакетОтбора");
	//Для каждого мОтбор Из ПараметрыОтчета.ПользовательскиеНастройкиКД.Элементы Цикл
	//	Если мОтбор.Использование Тогда
	//		ОбластьСтрока = ТабМакет.ПолучитьОбласть("Строка");
	//		
	//		Для Каждого мЭлемент Из мОтбор.ПравоеЗначение Цикл
	//			ОбластьСтрока.Параметры.Площадка = мЭлемент.Значение.Наименование;
	//			ОбластьСтрока.Параметры.GUID = мЭлемент.Значение.GUID_Меркурий;
	//			ОбластьСтрока.Параметры.Наименование = Объект.Наименование;
	...
	//	Для Каждого Информация Из Получатель.Получатель.КонтактнаяИнформация Цикл
	//		Если Информация.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты и Информация.Вид = Справочники.ВидыКонтактнойИнформации.EmailПользователя Тогда
	//			ОбластьСтрока = ТабМакет.ПолучитьОбласть("Строка");
	//			ОбластьСтрока.Параметры.GUID = Информация.АдресЭП;
	//			ОбластьСтрока.Параметры.Наименование = Объект.Наименование;
	//			ТабДок.Вывести(ОбластьСтрока);
	//		КонецЕсли;
	//	КонецЦикла;
